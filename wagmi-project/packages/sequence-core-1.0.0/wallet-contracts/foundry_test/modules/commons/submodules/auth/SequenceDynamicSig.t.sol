...
contract SequenceDynamicSigImp {
    ...
    function recoverBase(bytes32 _subdigest, bytes calldata _signature)
        external
        view
        returns (uint256 threshold, uint256 weight, bytes32 imageHash, uint256)
    {
...
contract SequenceDynamicSigTest is AdvTest {
    ...
    function test_recover_ignoreFirstByte(
        uint8 _first,
        bytes32 _subdigest,
        uint256 _pk,
        uint16 _threshold,
        ...
        bytes memory encoded =
            abi.encodePacked(_threshold, _checkpoint, uint8(0), _weight, signAndPack(_pk, _subdigest, 1));

        (uint256 threshold1, uint256 weight1, bytes32 imageHash1, uint256 checkpoint1) =
            lib.recover(_subdigest, abi.encodePacked(_first, encoded));
        (uint256 threshold2, uint256 weight2, bytes32 imageHash2, uint256 checkpoint2) =
            lib.recoverBase(_subdigest, encoded);
