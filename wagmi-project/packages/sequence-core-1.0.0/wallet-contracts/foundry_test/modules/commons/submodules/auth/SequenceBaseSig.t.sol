...
contract SequenceBaseSigImp {
    ...
    function recoverBranch(bytes32 _digest, bytes calldata _signature)
        external
        view
...
contract SequenceBaseSigTest is AdvTest {
    ...
    function test_subdigest(bytes32 _digest, uint256 _chainId) external {
        ...
        bytes32 expected = keccak256(abi.encodePacked("\x19\x01", _chainId, address(lib), _digest));
    ...
    function test_leafForAddressAndWeight_fuzz(address _addr1, uint96 _weight1, address _addr2, uint96 _weight2)
        external
    {
    ...
    function test_leafForNested_fuzz_subdigest(bytes32 _subdigest, bytes32 _node, uint256 _threshold, uint256 _weight)
        external
    {
    ...
    function test_recoverBranch_Addresses(bytes32 _subdigest, bytes32 _seed, address[] calldata _addresses) external {
        ...
        for (uint256 i = 0; i < size; i++) {
            uint8 randomWeight =
                uint8(bound(uint256(keccak256(abi.encode(_addresses[i], i, _seed))), 0, type(uint8).max));
            ...
            if (op == 0) {
                ...
                if (op == 1) {
                    ...
                    signature = abi.encodePacked(
                        signature, FLAG_DYNAMIC_SIGNATURE, randomWeight, addr, uint24(sigpart.length), sigpart
                    );
                }
            }
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            if (op == 0) {
                ...
                if (op == 1) {
                    ...
                    signature = abi.encodePacked(
                        FLAG_DYNAMIC_SIGNATURE, randomWeight, addr, uint24(sigpart.length), sigpart, signature
                    );
                }
            }
    ...
    function test_recover(bytes32 _subdigest, uint256 _pk, uint32 _checkpoint, uint16 _threshold, uint8 _weight)
        external
    {
        _pk = boundPk(_pk);
