...
contract LibOptimTest is AdvTest {
    ...
    function test_returnData_Fuzz(bytes memory _data) external {
        ...
        uint256 pointer2;
        assembly { pointer2 := mload(0x40) }
        assertEq(pointer2 - pointer1, res1.length + 32);

        uint256 positionArr;
        assembly { positionArr := optres }
        assertEq(positionArr, pointer1);
    }

    function test_call(address _to, uint256 _val, bytes calldata _data) external {
        _to = boundNoSys(_to);
