...
contract TrustTest is AdvTest {
    ...
    function test_fail_schedule_unlock_too_early(
        uint256 _ownerPk,
        ...
        vm.expectRevert(
            abi.encodeWithSignature("UnlockTooEarly(uint256,uint256)", block.timestamp + _schedule, _schedule)
        );
    ...
    function test_fail_schedule_in_the_past(
        ...
        vm.prank(vm.addr(boundPk(_ownerPk)));
        vm.expectRevert(
            abi.encodeWithSignature("UnlockInThePast(uint256,uint256)", block.timestamp - _schedule, _schedule)
        );
        trust.setUnlocksAt(block.timestamp - _schedule);
    ...
    function test_fail_schedule_non_allowed(
        uint256 _ownerPk,
        uint256 _beneficiaryPk,
        uint256 _badActorpk,
        uint256 _duration,
    ...
    function test_send_transaction_after_unlock(
        ...
        Trust t = gen_and_unlock(_ownerPk, _beneficiaryPk, _duration, _unlockAt, _extra);
    ...
    function test_fail_bubble_up_fail(
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "FailedTransaction(address,uint256,bytes,bytes)", address(mf), _value, _data, _revertData
            )
        );
