...
contract L2CompressorHuffReadExecuteTest is AdvTest {
    ...
    function setUp() public {
        imp = address(HuffDeployer.config().with_evm_version("paris").deploy("imps/L2CompressorReadExecute"));
    }
    ...
    function test_read_simple_execute(
        ...
        bytes memory encoded = abi.encodePacked(encodeWord(_space), encodeWord(_nonce), uint8(_txs.length));
        ...
        encoded = abi.encodePacked(encoded, encode_bytes_n(_signature));
    ...
    function test_read_nested_execute(
        ...
        bytes memory encoded = abi.encodePacked(
            encodeWord(bytes32(0)),
            encodeWord(bytes32(0)),
            uint8(1), // One transaction
            build_flag(
                outerTx[0].delegateCall,
                outerTx[0].revertOnError,
                outerTx[0].gasLimit != 0,
                outerTx[0].value != 0,
                outerTx[0].data.length != 0
            ),
            bytes(""),
        ...
        encoded = abi.encodePacked(encoded, encode_bytes_n(outerSignature));
        ...
        // Encode using solidity
        bytes memory solidityEncoded =
            abi.encodeWithSelector(IModuleCalls.execute.selector, outerTx, outerNonce, outerSignature);
        assertEq(solidityEncoded, res);
    }
}
