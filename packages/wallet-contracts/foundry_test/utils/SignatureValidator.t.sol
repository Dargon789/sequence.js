...
contract SignatureValidatorTest is AdvTest {
    ...
    function test_recoverSigner_ETHSIGN(uint256 _pk, bytes32 _hash) external {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(_pk, keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", _hash)));

        address recovered = lib.recoverSigner(_hash, abi.encodePacked(r, s, v, SIG_TYPE_ETH_SIGN));
        assertEq(signer, recovered);
    }

    function test_recoverSigner_fail_InvalidSValue(bytes32 _hash, bytes32 _r, uint256 _s, uint8 _v, uint8 _type)
        external
    {
    ...
    function test_recoverSigner_fail_InvalidVValue(bytes32 _hash, bytes32 _r, uint256 _s, uint8 _v, uint8 _type)
        external
    {
    ...
    function test_recoverSigner_fail_UnsupportedSignatureType(
        bytes32 _hash,
        bytes32 _r,
        uint256 _s,
        uint8 _v,
    ...
    function test_recoverSigner_fail_RecoverAddressZero(bytes32 _hash, bytes32 _r, uint256 _s, uint8 _v, uint8 _type)
        external
    {
        _s = bound(_s, 0, 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0);
        _v = uint8(bound(_v, 27, 28));
    ...
    function test_isValidSignature_Fail_WALLET_BYTES32_BadBytes4(
        address _wallet,
        bytes32 _hash,
    ...
    function test_isValidSignature_Fail_WALLET_BYTES32_BadReturn(
        address _wallet,
        bytes32 _hash,
        bytes calldata _signature,
    ...
    function test_isValidSignature_Fail_UnsupportedSignatureType(
        bytes32 _hash,
        address _signer,
        bytes calldata _signature,
        uint8 _type
        ...
        vm.expectRevert(
            abi.encodeWithSignature("UnsupportedSignatureType(bytes,uint256,bool)", encodedSignature, _type, false)
        );
