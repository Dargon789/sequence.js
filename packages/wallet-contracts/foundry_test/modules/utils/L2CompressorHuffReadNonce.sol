...
contract L2CompressorHuffReadNonceTest is AdvTest {
    ...
    function setUp() public {
        imp = address(HuffDeployer.config().with_evm_version("paris").deploy("imps/L2CompressorReadNonce"));
    }
    ...
    function test_read_simple_nonce() external {
        ...
        bytes memory encoded = abi.encodePacked(encodeWord(space), encodeWord(nonce));
    ...
    function test_read_nonce(uint160 _space, uint96 _nonce) external {
        bytes memory encoded = abi.encodePacked(encodeWord(_space), encodeWord(_nonce));
