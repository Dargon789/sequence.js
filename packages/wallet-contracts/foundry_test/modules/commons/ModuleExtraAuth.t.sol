...
contract ModuleExtraAuthImp2 is ModuleAuthImp, ModuleExtraAuth {
    function _isValidImage(bytes32 _imageHash) internal view override(ModuleAuthImp, ModuleExtraAuth) returns (bool) {
...
contract ModuleExtraAuthTest is AdvTest {
    ...
    function test_shouldClearExtraImageHashes(bytes32 _base, SetIh[] calldata _set, bytes32[] calldata _clear)
        external
    {
        ...
        for (uint256 i = 0; i < sizeSet; i++) {
            assertEq(imp.isValidImage(_set[i].imageHash), _set[i].imageHash == _base || !wasCleared[_set[i].imageHash]);
        }
    }

    function test_fail_setExtraImageHash_notSelf(address _caller, bytes32 _imageHash, uint256 _expiration) external {
    ...
    function test_fail_clearExtraImageHash_notSelf(address _caller, bytes32[] calldata _clear) external {
        boundDiff(_caller, address(imp));
        vm.expectRevert(abi.encodeWithSignature("OnlySelfAuth(address,address)", _caller, address(imp)));
        vm.prank(_caller);
