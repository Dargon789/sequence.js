...
abstract contract SequenceChainedSig is IModuleAuth, ModuleSelfAuth {
        ...
        returns (uint256 threshold, uint256 weight, bytes32 imageHash, bytes32 subdigest, uint256 checkpoint)
    {
        ...
        (threshold, weight, imageHash, subdigest, checkpoint) = signatureRecovery(_digest, _signature[rindex:nrindex]);
        ...
        while (rindex < _signature.length) {
            ...
            (
                    ...
                    nextCheckpoint
                ) = signatureRecovery(_hashSetImageHashStruct(imageHash), _signature[rindex:nrindex]);
