        ...
        unchecked {
            ...
            for (uint256 i = 0; i < size; i++) {
                ...
                if (gasleft() < gasLimit) {
                    ...
                    results[i].result =
                        abi.encodeWithSelector(IModuleCalls.NotEnoughGas.selector, i, gasLimit, gasleft());
                    break;
                }
                ...
                if (transaction.delegateCall) {
                    ...
                    (results[i].succeeded, results[i].result) =
                        transaction.target.delegatecall{gas: gasLimit == 0 ? gasleft() : gasLimit}(transaction.data);

                    results[i].gasUsed = initialGas - gasleft();
                } else {
                    uint256 initialGas = gasleft();

                    (results[i].succeeded, results[i].result) = transaction.target
                    .call{value: transaction.value, gas: gasLimit == 0 ? gasleft() : gasLimit}(transaction.data);
