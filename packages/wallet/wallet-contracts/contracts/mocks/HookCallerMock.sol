...
contract HookCallerMock {
    function callERC1155Received(address _addr) external {
        bytes4 result = IERC1155Receiver(_addr).onERC1155Received(address(this), msg.sender, 1, 2, msg.data);
    ...
    function callERC1155BatchReceived(address _addr) external {
        ...
        bytes4 result = IERC1155Receiver(_addr).onERC1155BatchReceived(address(this), msg.sender, ids, values, msg.data);
    ...
    function callERC721Received(address _addr) external {
        bytes4 result = IERC721Receiver(_addr).onERC721Received(address(this), msg.sender, 1, msg.data);
