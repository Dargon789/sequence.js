...
function build_flag(bool _delegateCall, bool _revertOnError, bool _hasGasLimit, bool _hasValue, bool _hasData)
    pure
    returns (uint8)
{
    uint8 res = 0;
...
function encode_abi_call(bytes4 _selector, bytes32 _v1, bytes32 _v2, bytes32 _v3, bytes32 _v4)
    pure
    returns (bytes memory)
{
    return abi.encodePacked(
        uint8(0x31), uint8(0x00), _selector, encodeWord(_v1), encodeWord(_v2), encodeWord(_v3), encodeWord(_v4)
    );
}

function encode_abi_call(bytes4 _selector, bytes32 _v1, bytes32 _v2, bytes32 _v3, bytes32 _v4, bytes32 _v5)
    pure
    returns (bytes memory)
{
    return abi.encodePacked(
...
function encode_dynamic_signature(uint8 _weight, address _signer, bytes memory _signature) pure returns (bytes memory) {
    return
        abi.encodePacked(uint8(0x44), uint8(_weight), encodeWord(uint256(uint160(_signer))), encode_bytes_n(_signature));
}

function encode_sequence_signature(bool _noChainId, uint256 _threshold, uint32 _checkpoint, bytes memory _tree)
    pure
    returns (bytes memory)
{
    uint8 flag;
...
function encode_abi_dynamic(bytes4 _selector, bool[] memory _isDynamic, bytes[] memory _values)
    pure
    returns (bytes memory)
{
