...
contract L2CompressorHuffTest is AdvTest {
    ...
    function setUp() public {
        imp = address(HuffDeployer.config().with_evm_version("paris").deploy("L2Compressor"));
    }
    ...
    function test_execute(
        ...
        bytes memory encoded = abi.encodePacked(hex"00", encodeWord(_space), encodeWord(_nonce), uint8(_txs.length));
        ...
        vm.expectCall(
            ...
            0,
            abi.encodeWithSelector(IModuleCalls.execute.selector, _txs, packedNonce, _signature)
        );
    ...
    function test_execute_many(BatchMember[] memory _batch) internal {
        ...
        bytes memory encoded = abi.encodePacked(hex"01", uint8(size));
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            encoded = abi.encodePacked(
                encoded, encodeWord(_batch[i].space), encodeWord(_batch[i].nonce), uint8(_batch[i].txs.length)
            );
            ...
            encoded = abi.encodePacked(encoded, encode_bytes_n(_batch[i].signature), encodeWord(uint256(uint160(addr))));

            vm.expectCall(
                addr,
                0,
                abi.encodeWithSelector(IModuleCalls.execute.selector, _batch[i].txs, packedNonce, _batch[i].signature)
            );
        }
    ...
    function test_read_storage_slots() external {
        ...
        (bool s, bytes memory r) =
            imp.call(abi.encodePacked(hex"05", uint256(0) << 128, uint256(1), uint256(1) << 128, uint256(2)));
    ...
    function test_decode_execute(
        ...
        bytes memory encoded = abi.encodePacked(hex"06", encodeWord(_space), encodeWord(_nonce), uint8(_txs.length));
    ...
    function test_decode_execute_many(BatchMember[] memory _batch) internal {
        ...
        bytes memory encoded = abi.encodePacked(hex"07", uint8(size));
