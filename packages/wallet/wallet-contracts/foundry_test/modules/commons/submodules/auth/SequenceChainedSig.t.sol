...
contract SequenceChainedSigImp is SequenceChainedSig {
    function pchainedRecover(bytes32 _digest, bytes calldata _signature)
        external
        view
    ...
    function _signatureValidation(bytes32, bytes calldata) internal pure override returns (bool, bytes32) {
...
contract SequenceChainedSigTest is AdvTest {
    ...
    function test_chainedRecover(uint8 _prefix, bytes32 _digest, HashAndSignature[] memory _steps) external {
        ...
        (uint256 threshold, uint256 weight, bytes32 imageHash, bytes32 subdigest, uint256 rcheckpoint) =
    ...
    function test_chainedRecover_Fail_LowWeight(
        uint8 _prefix,
        uint256 _badi,
        bytes32 _digest,
        HashAndSignature[] memory _steps
    ) external {
        vm.assume(_steps.length > 0);
        ...
        vm.expectRevert(
            abi.encodeWithSignature(
                "LowWeightChainedSignature(bytes,uint256,uint256)",
                _steps[_badi].signature,
                _steps[_badi].threshold,
                _steps[_badi].weight
            )
        );
        lib.pchainedRecover(_digest, signature);
    ...
    function test_chainedRecover_Fail_Checkpoint(
        uint8 _prefix,
        uint256 _badi,
        bytes32 _digest,
        HashAndSignature[] memory _steps
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            if (i != 0) {
                if (_badi == i) {
                    ...
                    checkpoint =
                        uint64(bound(uint256(checkpoint) + _steps[i].checkpointDelta, checkpoint, type(uint64).max));
        ...
        if (_badi != 0) {
            vm.expectRevert(
                abi.encodeWithSignature(
                    "WrongChainedCheckpointOrder(uint256,uint256)", badCheckpoint, prevToBadCheckpoint
                )
            );
        }
