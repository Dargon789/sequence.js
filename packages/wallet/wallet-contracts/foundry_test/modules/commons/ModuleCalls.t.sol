...
contract WillRevert {
    ImmutableBytes private immutable ib;
...
contract WillDelegateTo {
    address private immutable expectSelf;
    bytes32 private immutable expectData;
    address private immutable delegate;
...
contract ModuleCallsImp is ModuleCalls {
    ...
    function signatureRecovery(bytes32, bytes calldata)
        public
        view
        override
        returns (uint256, uint256, bytes32, bytes32, uint256)
    {}
...
contract ModuleCallsTest is AdvTest {
    ...
    function test_execute(ToValAndData[] memory _rtxs, bytes memory _sig, bytes32 _subdigest) external {
        ...
        for (uint256 i = 0; i < size; i++) {
            ...
            txs[i].target = boundNoBalance(
                boundNoContract(
                    boundDiff(boundNoSys(_rtxs[i].target), address(template), address(imp), address(factory))
                )
            );
    ...
    function test_execute_delegateCall(ToValAndData[] memory _rtxs, bytes memory _sig, bytes32 _subdigest) external {
        uint256 size = mayBoundArr(_rtxs.length);
        IModuleCalls.Transaction[] memory txs = new IModuleCalls.Transaction[](size);
    ...
    function test_validateNonce_Normal(uint160 _space, uint88 _nonce) external {
        uint256 encoded = uint256(abi.decode(abi.encodePacked(_space, uint8(0), _nonce), (bytes32)));
    ...
    function test_fail_validateNonce_Normal_Bad(uint160 _space, uint88 _nonce, uint88 _badprev) external {
